...
// The Drifter Engine: a genre-agnostic, Western-boned pulp premise generator.
Stimulus.register("plot-generator", class extends Controller {
  static targets = [
    "drifter", "code", "edge", "system", "enforcer", "squeeze", "job",
    "premise",
    "expansion", "reversal", "oneWorthSaving", "turn", "reckoning", "price", "wildcard",
    "skin", "expandToggle", "clip", "copyBtn",
    "genreNeutral", "genreFantasy", "genreScifi", "toneAny", "toneDark", "toneHeroic",
  ];
  static values = { genre: { type: String, default: "neutral" }, tone: { type: String, default: "any" } };
  ...
  TONES = {
    any: "",
    dark: " Play it dark: the Squeeze is cruel, the Reversal cuts deep, the Price steep.",
    heroic: " Play it light: more heroic, the Price gentler. The chassis holds; only the lean changes.",
  };
  ...
  LEGEND = [
    ...
    "Drifter = protagonist; rootless hero, free of competing obligations.",
    ...
    "Job = protagonist's external goal = the SEQ Want.",
    "Reversal = a hidden plot betrayal. NOT the SEQ backstory Wound.",
    ...
    "Reckoning = climax; one shot at the goal.",
    "Price = the ending's cost / thematic statement.",
  ...
  reroll(event) {
    ...
    const banks = {
      drifter: this.DRIFTER, code: this.CODE, edge: this.EDGE,
      ...
      reckoning: this.RECKONING, price: this.PRICE, wildcard: this.WILDCARD,
    };
  ...
  // values, the genre/tone note, and the standing legend — so whatever is
  // pasted elsewhere needs no inference about what each slot means.
  buildCopyText() {
    ...
    const lines = [
      ...
      `Wildcard: ${c.wildcard}`,
      "",
      `Genre: ${this.GENRE_LABELS[this.genreValue]} · Tone: ${this.TONE_LABELS[this.toneValue]} · Skeleton woven into premise: ${expanded ? "yes" : "no"}`,
      "",
      this.LEGEND,
    ];
    return lines.join("\n");
  ...
  fillExpansion() {
    ...
    this.current.price = this.pick(this.PRICE);
    ...
    this.priceTarget.textContent = this.current.price;
  ...
  renderSkin() {
    ...
    this.skinTarget.textContent =
      (this.SKINS[this.genreValue] || this.SKINS.neutral) + (this.TONES[this.toneValue] || "");
    this.highlightDials();
  }

  // Mark the active genre/tone pill via aria-pressed (styled in the page CSS).
  highlightDials() {
    const genrePills = {
  ...
  renderSeed() {
    this.drifterTarget.textContent = this.current.drifter;
  ...
  // Weave the seed slots into a logline; when the toggle is on, append the
  // expanded skeleton (Reversal, One Worth Saving, Turn, Reckoning, Price, Wildcard).
  renderPremise() {
    if (!this.current) return;
    const c = this.current;
    let text =
      `${this.capitalize(c.drifter)} reaches ${c.edge}, where ${c.system}, working through ${c.enforcer}, ` +
      `is ${c.squeeze}. The drifter, who holds that ${c.code}, takes one job: to ${c.job}.`;
    if (this.hasExpandToggleTarget && this.expandToggleTarget.checked && c.reversal) {
      text +=
        ...
        `and the price is ${c.price}. Wildcard: ${c.wildcard}.`;
    }
  ...
  // SYSTEM = the antagonist force, larger than any one person (the old "Power").
  SYSTEM = [
  ...
  // NOT a backstory injury (that internal-arc "Wound" is intentionally absent).
  REVERSAL = [
